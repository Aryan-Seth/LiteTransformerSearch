...
class TorchNumParameters(ModelEvaluator):
    ...
    def evaluate(self, arch: ArchaiModel, budget: Optional[float] = None) -> float:
        total_params = sum(
            param.numel() for param in arch.arch.parameters() if not self.trainable_only or param.requires_grad
        )
        ...
        exclude_params = (
            ...
            if self.exclude_cls is None
                ...
                for module in arch.arch.modules()
...
class TorchFlops(ModelEvaluator):
    ...
    def __init__(
        ...
        Args:
            forward_args: `arch.forward()` arguments used for profilling.
            forward_kwargs: `arch.forward()` keyword arguments used for profilling.
...
class TorchLatency(ModelEvaluator):
    """Average/median latency (in seconds) of a PyTorch arch using a sample input."""
...
class TorchPeakCudaMemory(ModelEvaluator):
    """Measures CUDA peak memory (in bytes) of a PyTorch arch using a sample input."""
...
class TorchPeakCpuMemory(ModelEvaluator):
    """Measures CPU peak memory (in bytes) of a PyTorch arch using a sample input."""
    ...
    def evaluate(self, arch: ArchaiModel, budget: Optional[float] = None)->float:
        ...
        is_training = arch.arch.training
        arch.arch.eval()
        ...
        with torch.profiler.profile(
            ...
            with torch.profiler.record_function("model_inference"):
                arch.arch(*forward_args, **forward_kwargs)
