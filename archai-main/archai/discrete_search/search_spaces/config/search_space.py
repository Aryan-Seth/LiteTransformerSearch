...
class ConfigSearchSpace(EvolutionarySearchSpace, BayesOptSearchSpace):
    def __init__(
        ...
        Args:
            model_cls (Type[torch.nn.Module]): arch class. This class expects that the first argument
    ...
    def save_arch(self, model: ArchaiModel, file_path: str) -> None:
        model.metadata["config"].to_file(file_path)

    @overrides
    def load_arch(self, file_path: str) -> ArchaiModel:
        config = ArchConfig.from_file(file_path)
        arch = self.model_cls(config, **self.model_kwargs)

        return ArchaiModel(arch=arch, archid=self.get_archid(config), metadata={"config": config})

    @overrides
    def save_model_weights(self, model: ArchaiModel, file_path: str) -> None:
        torch.save(model.arch.get_state_dict(), file_path)
    ...
    def crossover(self, arch_list: List[ArchaiModel]) -> ArchaiModel:
        ...
        model_1, model_2 = self.rng.choices(arch_list, k=2)
